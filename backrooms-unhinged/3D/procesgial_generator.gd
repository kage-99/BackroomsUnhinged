...
func _get_random_howler_pos() -> Vector2i:
	...
	while not valid_pos and attempts < 100:
		...
		if dist_to_player >= 3 and dist_to_exit >= 1:
			valid_pos = true

	print("[Generator] Howler-Spawn-Gitter nach %d Versuchen gefunden: %s" % [attempts, random_pos])
	return random_pos
...
func _simulate_backrooms_2d() -> void:
	...
	print("[Generator] Zufällige Innenwände platziert (%d Wände erzeugt)." % added_walls)
...
func _ensure_path_and_get_exit() -> Vector2i:
	...
	print("[Generator] Pfad zum Ausgang freigeräumt (%d Wände entfernt)." % cleared_walls)
		...
		for z in range(1, map_depth - 1):
			if rng.randf() < 0.2:
				if rng.randf() < 0.5 and h_walls[x][z]:
					...
					v_walls[x][z] = false
					loop_count += 1
	print("[Generator] Schleifen/Durchgänge hinzugefügt (%d Durchbrüche)." % loop_count)

# --- 3D BUILDER UNDER NAVIGATION REGION ---

func _clear_map() -> void:
	if nav_region:
		var count := nav_region.get_child_count()
		for child in nav_region.get_children():
			child.queue_free()
		print("[Generator] Alte Welt-Objekte gelöscht (%d Instanzen)." % count)

func _spawn_roof() -> void:
	if ground_scene == null:
		push_warning("[Generator] Abbruch: 'ground_scene' ist nicht im Inspector zugewiesen!")
		return
	...
	ground.position = Vector3(map_total_width / 2.0, 5.80, map_total_depth / 2.0)
	print("[Generator] Decke (Roof) platziert auf Y = 5.80.")
...
func _spawn_ground() -> void:
	...
	print("[Generator] Boden (Ground) platziert auf Y = 0.00.")
...
func _build_3d_walls() -> void:
	...
	print("[Generator] Insgesamt %d Wände als 3D-Instanzen aufgebaut." % total_spawned_walls)

func _spawn_wall_instance(pos: Vector3, rot_y: float) -> void:
	if wall_scene == null:
		push_warning("[Generator] 'wall_scene' ist nicht gesetzt!")
		return
...
func _bake_nav_mesh() -> void:
	if nav_region and nav_region.navigation_mesh:
		print("[Generator] Starte Baken des NavigationMesh...")
		nav_region.bake_navigation_mesh(true)
		print("[Generator] NavMesh-Baking Vorgang angestoßen.")
	else:
		push_warning("[Generator] NavMesh konnte nicht gebacken werden: nav_region oder mesh fehlt.")
...
func _spawn_exit_ladder(exit_2d: Vector2i) -> void:
	...
	for side in [-0.35, 0.35]:
		...
		var box := BoxMesh.new()
		box.size = Vector3(0.06, 5.0, 0.06)
		pole.mesh = box
		pole.material_override = ladder_material
		pole.position = Vector3(side, 2.5, 0)
	...
	for step in range(10):
		...
		rung.position = Vector3(0, step * 0.45 + 0.3, 0)
		exit_node.add_child(rung)

	var static_body := StaticBody3D.new()
	var collision := CollisionShape3D.new()
	var box_shape := BoxShape3D.new()
	box_shape.size = Vector3(0.8, 5.0, 0.2)
	...
	exit_light.position = Vector3(0, 2.0, 0)
	exit_node.add_child(exit_light)

	print("[Generator] Ausstiegsleiter (Exit) instanziiert bei: ", exit_3d)
...
func _spawn_howler(howler_2d: Vector2i) -> void:
	...
	if howler_node:
		...
		print("[Generator] Howler-Position gesetzt auf 3D-Koordinate: ", howler_3d)
	else:
		push_warning("[Generator] Howler Node im Scene-Tree nicht gefunden! ('../Howler')")
